...
def test_enhancers_gwilliams() -> None:
    STUDY_FOLDER = Path(os.environ.get("BRAINAI_STUDIES_PATH", "/large_experiments/brainai/shared/studies/"))
    # AWS cluster
...
def test_add_text_to_words():
    _, df = _make_test_events()
    df = enhancers.AddText()(df)
    assert "Text" in df.type.unique()
    text = df[df.type == "Text"].iloc[0].text
    assert len(text.split(".")) == 3


def test_remove_gaps():
    _, df = _make_test_events()
    df = enhancers.RemoveGaps()(df)
    durations = df.duration.values
    assert all(durations[:-1] == 1)
