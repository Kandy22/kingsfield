...
@dataclasses.dataclass
class MultiSegmentData:
    """Same as SegmentData but the segments can differ for each feature"""

    data: tp.Dict[str, torch.Tensor]
    segments: tp.Dict[str, tp.List[ns.segments.Segment]]
    ...
    def __post_init__(self) -> None:
        ...
        if not self.data:
            raise ValueError(f"No data in {self}")
        for _, segments in self.segments.items():
            if not isinstance(segments, list):
                raise TypeError(f"'segments' needs to be a list, got {segments}")
            # check batch dimension
            batch_size = next(iter(self.data.values())).shape[0]
            if len(segments) != batch_size:
                raise RuntimeError(
                    f"Incoherent batch size {batch_size} for {len(segments)} segments in {self}"
                )

    def to(self, device: str) -> "MultiSegmentData":
        """Creates a new instance on the appropriate device"""
        out = {name: d.to(device) for name, d in self.data.items()}
        return MultiSegmentData(data=out, segments=self.segments)
...
class MultiSegmentDataset(SegmentDataset):
    """
    A SegmentDataset where each feature is applied to segments with different start and duration.
    For example, we may want to predict one TR of fMRI from a certain window of stimulus, e.g. start=-10 duration=4.

    Usage:
    -----
    .. code-block:: python

        segments1 = ns.segments.list_segments(events, stride=1/TR, start=0, duration=TR)
        segments2 = ns.segments.list_segments(events, stride=1/TR, start=-10, duration=4)
        neuro, feature = Fmri(), Video()
        dataset = MultiSegmentDataset(
            features = {"neuro":neuro, "feature": feature},
            segments = {"neuro": segments1, "feature": segments2}
        )
    """
    ...
    def __init__(
        self,
        features: tp.Mapping[str, Feat],
        segments: tp.Mapping[str, tp.Sequence[ns.segments.Segment]],
        pad_duration: float | None = None,
        remove_incomplete_segments: bool = False,
    ) -> None:
        if not set(features.keys()) == set(segments.keys()):
            raise ValueError("Features and segments keys must match")
        num_segments = [len(segs) for segs in segments.values()]
        if len(set(num_segments)) > 1:
            raise ValueError("All segment lists must have the same length")
        self.datasets = {}
        invalid_indices = set()
        for name, segs in segments.items():
            invalid_indices.update(
                find_incomplete_segments(segs, features[name]._event_types_helper.classes)
            )
        if invalid_indices:
            if remove_incomplete_segments:
                msg = f"Removing {len(invalid_indices)} segments out of {len(segments)}"
                for name, segs in segments.items():
                    segments[name] = [  # type: ignore
                        s for i, s in enumerate(segs) if i not in invalid_indices
                    ]
            else:
                msg = f"{len(invalid_indices)} segments are missing some event types. Use `remove_incomplete_segments=True` to remove"
                raise ValueError(msg)
        for name, segs in segments.items():
            self.datasets[name] = SegmentDataset(
                {name: features[name]},
                segs,
                pad_duration,
    ...
    def __getitem__(self, idx: int) -> MultiSegmentData:  # type: ignore
        data, segments = {}, {}
        for name, dataset in self.datasets.items():
            segment_data = dataset[idx]
            data[name] = segment_data.data[name]
            segments[name] = segment_data.segments
        return MultiSegmentData(data=data, segments=segments)

    def collate_fn(self, batches: tp.List[MultiSegmentData]) -> MultiSegmentData:  # type: ignore
        """Creates a new instance from several by stacking in a new first dimension
        for all attributes
        """
        if not batches:
            return MultiSegmentData(data={}, segments={})
        if len(batches) == 1:
            return batches[0]
        if not batches[0].data:
            raise ValueError(f"No feature in first batch: {batches[0]}")
        ...
        for name in self.datasets:
            ...
            try:
                ...
                logger.warning(string)
                raise
            segments[name] = [s for b in batches for s in b.segments[name]]
        return MultiSegmentData(data=features, segments=segments)
