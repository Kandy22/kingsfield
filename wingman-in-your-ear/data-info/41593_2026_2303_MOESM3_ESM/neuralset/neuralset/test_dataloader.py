...
def test_as_one_batch_order() -> None:
    ...
    feats = {"stim1": stim, "stim2": stim}
    ds = dl.SegmentDataset(feats, segments).as_one_batch(num_workers=8)
...
class MultiSegmentDataLoader(pydantic.BaseModel):
    studies: tp.List[ns.data.StudyLoader]
    features: tp.Mapping[str, ns.features.FeatureConfig]
    event_type: str = "Image"
    start: tp.Mapping[str, float]
    duration: tp.Mapping[str, float]
    batch_size: int = 1
    num_workers: int = 0
    remove_incomplete_segments: bool = False

    def build(self) -> DataLoader[dl.MultiSegmentData]:
        if not (
            set(self.start.keys())
            == set(self.features.keys())
            == set(self.duration.keys())
        ):
            raise ValueError("Start, duration and features must have the same keys")
        all_studies = pd.concat([s.build() for s in self.studies])
        segments = {}
        for name, feature in self.features.items():
            feature.install_requirements()
            feature.prepare(all_studies)
            segments[name] = list_segments(
                all_studies,
                idx=all_studies.type == self.event_type,
                start=self.start[name],
                duration=self.duration[name],
            )
        ds = dl.MultiSegmentDataset(
            self.features,
            segments,
            remove_incomplete_segments=self.remove_incomplete_segments,
        )
        return ds.build_dataloader(
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            persistent_workers=self.num_workers > 0,
        )  # type: ignore
...
def test_multisegment_dataloader(tmp_path: Path) -> None:
    dloader = MultiSegmentDataLoader(
        studies=[{"name": "TestMeg2023", "path": tmp_path}],  # type: ignore
        features={
            "pulse": {"name": "Pulse", "frequency": 100, "aggregation": "sum"},  # type: ignore
            "meg": {"name": "Meg", "frequency": 100},  # type: ignore
        },
        start={"pulse": 0.0, "meg": -1},
        duration={"pulse": 1.0, "meg": 3.0},
        batch_size=2,
    )
    b = next(iter(dloader.build()))
    assert len(b.segments) == 2
    assert b.data["pulse"].shape == (2, 1, 100)
    assert b.data["meg"].shape == (2, 22, 300)


def test_multisegment_incomplete_segments(tmp_path: Path) -> None:
    dloader = MultiSegmentDataLoader(
        studies=[{"name": "TestMeg2023", "path": tmp_path}],  # type: ignore
        features={
            "pulse": {"name": "Pulse", "frequency": 100, "aggregation": "sum"},  # type: ignore
        ...
        start={"pulse": -100, "meg": 0},
        duration={"pulse": 1.0, "meg": 1.0},
        batch_size=2,
    )
    with pytest.raises(ValueError):
        next(iter(dloader.build()))
    dloader.remove_incomplete_segments = True
