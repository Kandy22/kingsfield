...
class MultiStudyLoader(pydantic.BaseModel):
    ...
    _studies: list[StudyLoader] = pydantic.PrivateAttr(default_factory=list)
    ...
    def model_post_init(self, log__: tp.Any) -> None:
        ...
        for name in self.names:
            kwargs["name"] = name
            self._studies.append(StudyLoader(**kwargs))

    def study_summary(self, apply_query: bool = True) -> pd.DataFrame:
        summaries = []
        for name, study in zip(self.names, self._studies):
        ...
        out.rename(
            columns={
                "timeline_index": "study_timeline_index",
                "subject_index": "study_subject_index",
                "subject_timeline_index": "study_subject_timeline_index",
            },
            inplace=True,
        )
        groups = out.groupby("subject")
