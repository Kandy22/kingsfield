...
class TextChunk(Text):
    """Event representing a text chunk with a specific content."""

    text: str

    _char_map: tp.ClassVar[tp.Dict[str, int]] = {
        "s": 0,
        "o": 1,
        "t": 2,
        "e": 3,
    ...
    def model_post_init(self, log__: tp.Any) -> None:
        ...
        #     if char not in self._char_map and not char.isdigit():
        #         raise ValueError(f"Text contains invalid character '{char}' not in character map.")

    @property
    def text_classes(self) -> torch.Tensor:
        indices = [
            (
                self._char_map.get(char, self._char_map["<special>"])
                if not char.isdigit()
                else self._char_map["<number>"]
            )
            for char in self.text
        ]
...
class Stimulus(Event):
    ...
    As opposed to events that point to actual stimulus data,
    `ns.events.Stimulus` only registers a `code` (or "trigger" value)
    that is mapped to an event.
    """
...
class ButtonEmg(Text):
    _button_map: tp.ClassVar[tp.Dict[str, int]] = {
        "t": 0,
        "h": 1,
        "e": 2,
        "<space>": 3,
    ...
    def model_post_init(self, log__: tp.Any) -> None:
        super().model_post_init(log__)
        if self.text not in self._button_map:
            raise ValueError(
                f"Button class must be one of {list(self._button_map.keys())}, got {self.text}"
            )

    @property
    def button_class(self) -> int:
        return self._button_map[self.text]


class Window(Event):
    labels: str

    _labels_map: tp.ClassVar[tp.Dict[str, int]] = {
        "s": 0,
        "o": 1,
        "t": 2,
        "e": 3,
    ...
    def model_post_init(self, log__: tp.Any) -> None:
        super().model_post_init(log__)
        if self.labels not in self._labels_map:
            raise ValueError(
                f"Label class must be one of {list(self._labels_map.keys())}, got {self.labels}"
            )

    @property
    def labels_class(self) -> int:
        return self._labels_map[self.labels]
...
class Ieeg(Meg):
    """Intracranial electroencephalography (iEEG) event; encompasses stereoelectroencephalography (sEEG) and electrocorticography (ECoG)"""
