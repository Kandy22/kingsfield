...
def test_chunk_events(tmp_path: Path) -> None:
    ...
    # Split the audio
    events2 = splitting.chunk_events(
        events, event_type_to_chunk="Sound", event_type_to_use="Word"
    )
    sounds = events2[events2["type"] == "Sound"]
    assert len(sounds) == 11
    assert all(sounds.offset.values == list(range(11)))

    events3 = splitting.chunk_events(
        events, event_type_to_chunk="Sound", event_type_to_use="Word", min_duration=0.5
    )
    sounds = events3[events3["type"] == "Sound"]
    assert len(sounds) == 10

    events4 = splitting.chunk_events(events, event_type_to_chunk="Sound", max_duration=2)
    sounds = events4[events4["type"] == "Sound"]
    assert len(sounds) == 6

    events5 = splitting.chunk_events(
        events,
        event_type_to_chunk="Sound",
        max_duration=2,
        min_duration=0.5,
    )
    sounds = events5[events5["type"] == "Sound"]
    assert len(sounds) == 5
    ...
    mock_events = pd.DataFrame(
        {
            ...
            "filepath": [str(filepath) for filepath in image_fpaths]
            + [None],  # Convert Path to str
        }
    )

    # Call the splitter with the DataFrame
    feat = ns.features.image.HuggingFaceImage(device="cpu", layers=1.0)
    splitter = splitting.SimilaritySplitter(feature=feat)  # TODO fix for layers != 1.0
