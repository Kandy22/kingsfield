...
def test_event_from_dict_with_nan() -> None:
    data = {
        ...
        "extra__category": "verb",
        "extra__whatever": np.nan,
    }
    word = ns.events.Event.from_dict(data)
    assert word.extra == {"category": "verb", "blublu": "hello"}
    out = word.to_dict()
    assert out["text"] == ""
...
def test_text_chunk() -> None:
    ...
    assert isinstance(event, ns.events.TextChunk)
...
def test_event_type_list() -> None:
    ...
    for fp in Path(__file__).with_name("studies").glob("**/*.py"):
        ...
        if not fp.name.startswith("test_") and fp.exists():
            stem = str(fp.stem)
            if fp.parent.stem == "staging":
                stem = "staging." + stem
            # sometimes new files make a mess with editable_mode=strict install
    ...
    msg += " if this is an intended change, so as to reference existing event classes "
    msg += (
...
# @pytest.mark.parametrize("key", ("a", "<space>"))
# def test_emg_button(key: str) -> None:
#     event = ns.events.Event.from_dict(
#         dict(
#             type="Button",
#             start=1,
...
#         )
#     )
#     assert isinstance(event, ns.events.ButtonEmg)
#     assert event.text == key
