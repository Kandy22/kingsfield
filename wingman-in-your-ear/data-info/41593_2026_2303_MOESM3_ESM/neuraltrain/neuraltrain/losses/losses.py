...
class MultiLoss(nn.Module):
    ...
    def forward(
        ...
        """
        loss_values = {"total": torch.tensor(0.0, device=self.weights.device)}
        for (name, loss_fn), weight in zip(self.losses.items(), self.weights):
            _x = x[name] if isinstance(x, dict) else x
            _y = y[name] if isinstance(y, dict) else y
            loss_values[name] = loss_fn(_x, _y)
            loss_values["total"] += weight * loss_values[name]
