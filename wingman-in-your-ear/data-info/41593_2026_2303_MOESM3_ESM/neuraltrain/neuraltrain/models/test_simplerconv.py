...
from .common import ChannelMerger
from .simplerconv import SimplerConv, SimplerConvConfig
...
def test_simpler_conv(fake_eeg, use_default_config):
    ...
    n_outputs = 512
    model = SimplerConvConfig(**config_kwargs).build(n_in_channels, n_outputs=n_outputs)
    assert isinstance(model, SimplerConv)
    if not use_default_config:
        assert isinstance(model.merger, ChannelMerger)
