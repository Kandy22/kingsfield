...
class TemporalDownsamplingConfig(BaseModelConfig):
    name: tp.Literal["TemporalDownsampling"] = "TemporalDownsampling"  # type: ignore
    ...
    def build(self, dim: int) -> nn.Module:
        return TemporalDownsampling(dim=dim, config=self)
...
class TemporalDownsampling(nn.Module):
    def __init__(
        self,
        dim: int,
        config: TemporalDownsamplingConfig | None = None,
    ) -> None:
        config = config if config is not None else TemporalDownsamplingConfig()
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        assert x.ndim == 4, f"Input must be of shape (B, 1, T, F), got {x.shape}"
        x = self.agg(x)
        if self.layer_norm:
            x = self.layer_norm(x)
        if self.gelu:
