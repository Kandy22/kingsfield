...
(see pinet_decoding/callbacks.py). This script reads those JSON files,
...
CHAR_MAP = {
    ...
    8: " ", 9: "d", 10: "l", 11: "r", 12: "b", 13: "@", 14: "z", 15: "v",
    16: "f", 17: "m", 18: "u", 19: "h", 20: "p", 21: "g", 22: "q", 23: "w",
    24: "x", 25: "y", 26: "j", 27: "k", 28: "9",
}
...
def process_json(json_path):
    ...
    df = pd.DataFrame(rows).sort_values(["Subject", "Sentence_UID"]).reset_index(drop=True)
    return df
...
def print_summary(df):
    ...
    for subject in sorted(df["Subject"].unique()):
        ...
        print(f"{subject:<12} {len(sdf):>12} {sdf['CER'].mean():>10.3f} {sdf['WER'].mean():>10.3f}")

    print("-" * 46)
    per_subj_cer = df.groupby("Subject")["CER"].mean()
    per_subj_wer = df.groupby("Subject")["WER"].mean()
    print(f"{'Overall':<12} {len(df):>12} {per_subj_cer.mean():>10.3f} {per_subj_wer.mean():>10.3f}")
    print(f"  (n = {df['Subject'].nunique()} subjects)\n")


def main():
    parser = argparse.ArgumentParser(description="Extract predictions from LogPreds callback JSON")
    parser.add_argument("--input", required=True, help="Path to callbacks directory or JSON file")
    parser.add_argument("--output", default=None, help="Output CSV path (default: <input>/predictions.csv)")
    ...
    output_path = args.output or os.path.join(os.path.dirname(json_path), "predictions.csv")
