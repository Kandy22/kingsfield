    ...
    def step(self, logits: torch.Tensor):
        ...
        for hyp in self.beam:
            ...
            for idx in top_indices:
                ...
                lm_score = self.lm.BaseScore(hyp.lm_state, char, new_state) #- baseline_lm_score

                # lm_score = state.BaseScore(state, new_char, prev)
...
def compute_cer_wer(decoder, inputs, labels):
    ...
    for i, (emissions, true) in enumerate(zip(inputs, labels)):
        ...
        cer_beam = (Levenshtein.distance(true, decoded) / len(true))
        true_words, beam_words = true.split(" "), decoded.split(" ")
        wer_beam = Levenshtein.distance(true_words, beam_words) / len(true_words)
...
if __name__ == "__main__":
    predictions_df = pd.read_csv(os.path.join(DATA_ROOT, "prediction_csv", "reviews_EEG_2step.csv"), index_col=0)
    ...
    model = kenlm.Model(os.path.join(DATA_ROOT, "lm_arpa_files", "news_9gram.arpa"))
    ...
    predictions_df.to_csv(os.path.join(DATA_ROOT, "prediction_csv", "reviews_EEG_2step_LM.csv"))
    # np.save(os.path.join(DATA_ROOT, "temp_data", "SentenceLevel_EEG_Ours_CER.npy"), CER_results)
    # np.save(os.path.join(DATA_ROOT, "temp_data", "SentenceLevel_EEG_Ours_WER.npy"), WER_results)

    predictions_df = pd.read_csv(os.path.join(DATA_ROOT, "prediction_csv", "reviews_MEG_2step.csv"), index_col=0)
    inputs, labels = df_preprocessing(predictions_df, len(predictions_df))
    print(len(inputs), len(labels))
    predictions_df['Subject'].unique().sort()
    print(predictions_df['Subject'].nunique())
    ...
    predictions_df.to_csv(os.path.join(DATA_ROOT, "prediction_csv", "reviews_MEG_2step_LM.csv"))