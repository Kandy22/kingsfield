...
from . import utils
from .callbacks import JitterWindows, LogPreds
from .grids.utils import ShuffledSegmentDataset
...
def preprocessing(events):
    ...
    # Sentence Event Modification: one UID / sentence and duration, stop, max mapping to first and last buttons
    # sentences = events[
    #     (events["type"] == "Sentence") & (events["is_image"] == False)
    # ].copy()
    # sentences_uids = sentences["sentence_UID"].unique()
    # sentences = sentences.drop_duplicates(subset="sentence_UID")
    # sentences_uids = sentences["sentence_UID"].unique()
...
class Data(pydantic.BaseModel):
    ...
    def build(self) -> tuple[dict[str, DataLoader], int]:
        ...
        if self.timeline_study:
            np.random.seed(33)
            timelines = events["timeline"].unique()
            selected_timelines = np.random.choice(timelines, self.n_samples, replace=False)
            events = events[events["timeline"].isin(selected_timelines)]

        if self.subject_study:
            #np.random.seed(33)
            subjects = events["subject"].unique()
            selected_subjects = np.random.choice(subjects, self.n_subjects, replace=False)
            events = events[events["subject"].isin(selected_subjects)]
        
        if self.sentence_study:
            #np.random.seed(33)
            buttons = events[events["type"] == "Button"]
            sentences = buttons["sentence"].unique()
            selected_sentences = np.random.choice(sentences, self.n_sentences, replace=False)
            filtered_buttons = buttons[buttons["sentence"].isin(selected_sentences)]
            timelines_to_keep = filtered_buttons["timeline"].unique()
            events = events[events["timeline"].isin(timelines_to_keep)]
            events = events[~((events['type'] == 'Button') & (~events['sentence'].isin(selected_sentences)))]
        ...
        if self.second_step: # Add detected Buttons
            events = utils.add_detected_buttons(events, self.study.name)
        ...
        for split, batch_size in batch_sizes.items():
            ...
            if self.second_step and split in ["val", "test"]:
                split_mask = (events.split == split) & (events.type == "DetectedButton")
            ...
            loaders[split] = DataLoader(
                ...
                shuffle=False,  # Because of the transformers
...
class Experiment(BaseExperiment):
    ...
    def _init_module(
        self, model: nn.Module, transformer: nn.Module
    ) -> pl.LightningModule:
        ...
        pl_module = init_fn(
            ...
            second_step=self.data.second_step,
        )
    ...
    def _setup_trainer(self) -> pl.Trainer:
        callbacks = [
            LogPreds(),
            #StoreTransformerEmbeddings(),
            #StoreEmbeddings(),
            #IntermediaryEmbeddingStore(),
        ]

        if self.data.second_step:
            callbacks.append(JitterWindows(fs=self.data.neuro.frequency))
            if self.save_checkpoints:
                callbacks.append(
                    ModelCheckpoint(
                        ...
                        filename="best",
                        monitor="train_transformer_loss",
                        mode="min",
                        save_on_train_epoch_end=True,
                    )
                )
        else:
            callbacks.append(EarlyStopping(monitor="val_CER", mode="min", patience=self.patience))
    ...
    def fit(
        ...
        if self.use_transformer:
            ...
            transformer = None
            brain_model = self.brain_model_config.build(
                n_in_channels=n_in_channels, n_outputs=29  # Change in n_hidden
            )
