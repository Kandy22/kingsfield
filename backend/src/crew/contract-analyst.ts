...
Keep each finding concise — total response must fit in 5000 tokens.
    `.trim(),
    maxTokens: 8192,
  });
  ...
  try {
    ...
    // Attempt bracket recovery on truncated output.
    try {
      parsed = JSON.parse(recoverTruncatedJson(json));
    } catch {
      console.error('[contract-analyst] JSON parse failed, using empty findings');
    }
  }
